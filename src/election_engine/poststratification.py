...
def compute_all_lga_results(
    ...
    _lga_turnout_mod = np.full(n_lgas, 0.40, dtype=np.float64)
    ...
    _lga_turnout_mod += 0.30 * _urban_sat  # saturated areas: comfort → apathy

    # Floor: every LGA has at least minimal abstention pressure (logistics, apathy)
    _lga_turnout_mod = np.maximum(_lga_turnout_mod, 0.25)
